...
Public Class frmLoader
    ...
    Private Sub frmLoader_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim failedCount = 0
        Dim cacheTask = Task.Run(Function() RunCoverCaching(pgbLoader, lblStatus, rtbLoaderLogs, failedCount))
    ...
    Private Sub OnCachingComplete(failedCount As Integer)
        If failedCount > 0 Then
            LogRichText(rtbLoaderLogs, $"✗ {failedCount} cover(s) failed to download after 3 attempts.", Color.Red)
            MessageBox.Show(
                $"{failedCount} image(s) failed to download after3 attempts.{vbCrLf}Their covers may be missing.",
                "Cover Download Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End If

        LogRichText(rtbLoaderLogs, "▸ Starting web server on port 5050...", Color.Gray)
        StartWebServer()
        LogRichText(rtbLoaderLogs, "✓ Ready!", Color.Green)
